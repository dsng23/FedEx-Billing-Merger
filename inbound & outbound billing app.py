...
# Choosing to merge inbound or outbound files
...
if workflow == "Outbound":
    ...
    # Explanation dropdown for what "rows to skip" are
    ...
    # Text box for the user to name the final downloaded report
    ...
    # Only running the code if both files are successfully uploaded
    if file1 is not None and file2 is not None:
        ...
        if st.button("🚀 Process & Combine Outbound Files", type="primary", key="out_btn"):
            try:
                ...
                # Fix accidental spaces in column names
                ...
                # Clean up the Order Numbers (remove extra spaces) so they link together perfectly
                ...
                # Keep only the columns we actually need for the report
                target_cols_1 = ['Date Out', 'Inv No.', 'Cnee', 'Dest', 'Order Nbr', 'Header Items']
                ...
                # Fix grouped orders (e.g. Order1-Order2) by splitting them into individual rows
                ...
                # Drop rows that don't have a quantity
                if 'Qty' in df2_clean.columns:
                    df2_clean = df2_clean.dropna(subset=['Qty'])

                # Match and link the two files together using the Order Number
                ...
                # If an order fails to match, try to rescue it by matching the Item Code instead
                if 'Header Items' in df1_clean.columns and 'Item Code' in combined_df.columns:
                    ...
                    if not unmatched_body.empty:
                        ...
                        cols_to_fill = ['Date Out', 'Inv No.', 'Cnee', 'Dest']
                ...
                # Build the Unmatched Log for items that completely failed to find a match
                ...
                # Keep only the successfully matched items for the main report
                ...
                # Set the exact left-to-right order of the final columns
                final_column_order = [
                    'Date Out', 'Con Note', 'Inv No.', 'Cnee', 
                    'Dest', 'Temp', 'Qty', 'Mat Type'
                ]
                ...
                # Preview of data before downloading final merged report
                st.markdown('<div class="fedex-divider"></div>', unsafe_allow_html=True)
                ...
                # Set up the background file to be downloaded as Excel
                buffer = io.BytesIO()
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    # Build the Dest and Inv No Summary Table
                    ...
                    worksheet.cell(row=4, column=11).border = thick_border 
                    worksheet.cell(row=4, column=12).border = thick_border 

                    head_d = worksheet.cell(row=5, column=10, value="Dest")
                    head_i = worksheet.cell(row=5, column=11, value="Inv No.")
                    head_t = worksheet.cell(row=5, column=12, value="Total")
                    
                    head_d.font = Font(bold=True)
                    head_i.font = Font(bold=True)
                    head_t.font = Font(bold=True)
                    
                    head_d.border = thin_border
                    head_i.border = thin_border
                    ...
                    for dest, group in outbound_sums.groupby('Dest', sort=True):
                        ...
                        group = group.sort_values(by='Inv No.')
                        ...
                        # Add subtotal row for each destination
                        ...
                        if current_row > start_dest_row:
                            sub_formula = f"=SUM(L{start_dest_row}:L{current_row - 1})"
                        else:
                            sub_formula = "=0"
                            
                        sub_t = worksheet.cell(row=current_row, column=12, value=sub_formula)
                        
                        sub_v.font = Font(bold=True)
                        sub_t.font = Font(bold=True)
                        sub_v.border = thin_border
                        sub_t.border = thin_border
                        
                        subtotal_rows.append(f"L{current_row}")
                        current_row += 1

                    # Add final Grand Total row in this table
                    gt_v = worksheet.cell(row=current_row, column=10, value="Grand Total")
                    worksheet.cell(row=current_row, column=11, value="").border = thin_border 
                    
                    if subtotal_rows:
                        gt_formula = "=" + "+".join(subtotal_rows) 
                    else:
                        gt_formula = "=0"
                    ...
                    t2_head_v = worksheet.cell(row=t2_start_row + 1, column=11, value="Temp")
                    t2_head_t = worksheet.cell(row=t2_start_row + 1, column=12, value="Total")
                    ...
                    for temp_cat in temp_categories:
                        ...
                        sumifs_formula = f'=SUMIFS(G$2:G${max_data_row}, F$2:F${max_data_row}, K{t2_current_row})'
                        worksheet.cell(row=t2_current_row, column=12, value=sumifs_formula).border = thin_border
                        
                        t2_current_row += 1

                    # Write the Unmatched Log to a second sheet if errors were found
                    if not unmatched_df.empty:
                        ...
                        for idx, col in enumerate(unmatched_df.columns):
                            max_len = unmatched_df[col].astype(str).str.len().max()
                            max_len = max(15, max_len) if pd.notna(max_len) else 15
                ...
                # Present the final file for the user to download
                st.download_button(
...
# INBOUND WORKFLOW 
    ...
    # Only run the code if both files are successfully uploaded
    if file1 is not None and file2 is not None:
